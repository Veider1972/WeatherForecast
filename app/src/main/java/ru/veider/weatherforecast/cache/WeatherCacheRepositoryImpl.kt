...
import ru.veider.weatherforecast.data.*
...
class WeatherCacheRepositoryImpl : WeatherCacheRepository {
    ...
    override fun getCache(latitude: Double, longitude: Double): WeatherData {
        val weatherEntities : List<WeatherEntity> = db.weatherCacheDao()
            .getCache(latitude, longitude, getCleanedTime())
        val weatherEntity = weatherEntities[weatherEntities.size-1]
        return WeatherData(
            weatherEntity.now_time,
            Info(weatherEntity.latitude, weatherEntity.longitude),
            GeoObject(
                District(weatherEntity.district_id, weatherEntity.district_name),
                Locality(weatherEntity.locality_id, weatherEntity.locality_name),
                Province(weatherEntity.province_id, weatherEntity.province_name),
                Country(weatherEntity.country_id, weatherEntity.country_name)
            ),
            Fact(
                weatherEntity.temp,
    ...
    override fun checkCache(latitude: Double, longitude: Double): Boolean {
